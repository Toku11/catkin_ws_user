	...
	# Remember image
        ...
        if self.CRT.has_result:
          ...
          try:
            ...
            #self.image_pub.publish(self.bridge.cv2_to_imgmsg(cv2.resize(im_gray, (150, 75)) , "mono8"))
            self.msg.header.stamp = rospy.Time.now()
            self.msg.header.frame_id = self.frame_id # i.e. '/odom'
            self.msg.child_frame_id = self.child_frame_id # i.e. '/base_footprint'

            self.msg.pose.pose.position = Point(ret_t[0,0]/100,ret_t[1,0]/100, vec_r[2])#ret_t[2,0]/100)
