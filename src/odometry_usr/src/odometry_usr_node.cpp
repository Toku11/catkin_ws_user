...
void visualCallback(const nav_msgs::Odometry::ConstPtr& msg){
  ...
  thcam=msg->pose.pose.position.z;
...
void headingCallback(const std_msgs::Float32& msg)
{
  ...
  else
  {
   ...
   if(correct==true){
    ...
    if(xk[0]<3.7&&xk[0]>2.7&&xk[1]<3.64&&xk[1]>2.20&&xk[1]<3.0){
      float err;
      err=(0-msg.data);
      offset=0.7*offset+0.3*err;
      xk[0]=0.75*xk[0]+0.25*3.48;//2.43
      printf("punto 2\n");
    }else if(xk[0]<1.2&&xk[0]>-1.35&&xk[1]<5.8&&xk[1]>5){
      float err;
      err=(88-msg.data);
      offset=0.7*offset+0.3*err;
      xk[1]=0.75*xk[1]+0.25*5.39;//5.6
      printf("punto 3\n");
    }
    else if(xk[0]<-3.20&&xk[0]>-3.84&&xk[1]<2.65&&xk[1]>-1){
      float err;
      printf("punto 4\n");
      err=(177-msg.data);
      if (err>180)
     err=err-360;
      ...
      offset=0.7*offset+0.3*err;
      xk[0]=0.75*xk[0]+0.25*-3.54;//-3.33
    }else if(xk[0]<1&&xk[0]>-1.35&&xk[1]>-5.8&&xk[1]<-5){
      float err;
      printf("punto 5\n");
      err=(-93-msg.data);
      offset=0.7*offset+0.3*err;
      xk[1]=0.75*xk[1]+0.25*-5.47;//5.6
    }else if(xk[0]<3.6&&xk[0]>2.8&&xk[1]>-3.5&&xk[1]<-2.7){
      float err;
      printf("punto 6\n");
      err=(0-msg.data);
      offset=0.7*offset+0.3*err;
      xk[0]=0.75*xk[0]+0.25*3.28;
    }
    correct=false;
...
int main(int argc, char** argv){
  ...
  ros::Subscriber subAngle_ = n.subscribe("angle_lane",1,angleCallback);
  ...
  while(n.ok()){
    ...
    odom.twist.twist.angular.x=thcam;
