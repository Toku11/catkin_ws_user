...
public:
	...
	{	
		if (init_park==false){
			px_ini=px;
			py_ini=py;
			head_ini=filter_yaw*3.1416/180;
			k_idx=0;
			sp_rx[0]=px_ini+0.0*sin(head_ini);sp_rx[1]=px_ini;sp_rx[2]= px_ini-0.8*sin(head_ini); sp_rx[3]=px_ini;sp_rx[4]=px_ini+0.5*sin(head_ini);sp_rx[5]=px_ini;
			sp_ry[0]=py_ini+0.7*cos(head_ini);sp_ry[1]=py_ini;sp_ry[2]= py_ini+1.2*cos(head_ini); sp_ry[3]=py_ini;sp_ry[4]=py_ini+0.9*cos(head_ini);sp_ry[5]=py_ini;
			init_park=true;
			num_sp=5;
		}else{
	        ...
	        err_x=spx-px;
	        err_y=spy-py;
	        if(err_x<.07 && err_x>-.07 && err_y<.07 && err_y>-.07){			   
			   k_idx++; 
			   if(k_idx>=num_sp){
			   	k_idx=0;
			   }
