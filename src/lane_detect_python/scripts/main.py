  ...
  def callback(self,data):
    ...
    if (self.conta%4==0):
        ...
        for val in ls:
            if len(ordp[0])==0:#lista nueva no inicializad
                ...
                for i in range(0,len(ordp)):
                    if np.abs(val[0]-ordp[i][len(ordp[i])-1][0])<15 and np.abs(val[1]-ordp[i][len(ordp[i])-1][1])<=(rws[0]-rws[2]):
                        ordp[i].append(val)
                        flag=True
                        break
        ...
        for x in range(0,len(ordp)):
            if (ordp[x][0][1]>0) and (len(ordp[x])>14):##LIMITE Y y LIMITE DE TAMAÑO
                print("crucero!!!!!!!!!!")
    ...
    # perform RANSAC iterations
        ...
        if(self.park_flg==True):
            d=((rho+(value_neg[kk,1]/2+20)*change)/np.sin(theta))
        else:
