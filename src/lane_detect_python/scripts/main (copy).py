...
class image_converter:
  ...
  def callback(self,data):
    ...
    #print('rows=',rows)
    #print('rows=',cols)
    tam_crop=44#crop para carretera
    tam_crop2=0#crop para ancho
    crop_img = frame[rows-tam_crop:rows-9, tam_crop2:cols-tam_crop2]
    offsetx=80#centrar en imagen vista aérea    
    
    gray = cv2.cvtColor(crop_img,cv2.COLOR_BGR2GRAY)      
    (rows3,cols3) = gray.shape
    correct=40#que tan alargada de arriba se quiere corregir
    elong=160#que tanto alargado se quiere    
  ...
  #########################################################################3
    ...
    if value[ind1][1]<=0 and any(x>0 for x in value[:,1])==True: #linea izquierda y posible carril
        ...
        if len(value_neg>0):## que no este vacio el arreglo
            ...
            value_neg=value_neg[np.where(np.all([value_neg[:,1]>95,value_neg[:,1]<260],0)==True)]##restriccion tamaño carril
            if len(value_neg)>0:
...
############################################################
        ...
        if len(value_neg>0):
            ...
            value_neg=value_neg[np.where(np.all([value_neg[:,1]>95,value_neg[:,1]<260],0)==True)]
