...
class image_converter:
  ...
  def callback(self,data):
    ...
    #***********************
        ...
        n=0                  
        for x in range(0,len(ordp)):
            if (ordp[x][0][1]>40) and (len(ordp[x])>6):##LIMITE Y y LIMITE DE TAMAÑO
                value.append([x,(ordp[x][1][0]-centro),len(ordp[x]),ordp[x][1][0]])
      ...
      #########################################################################
        ...
        if len(value)>2:##tres carriles
            ...
            if lado==self.lado_inicial:
                ...
                if len(np.where(np.sign(value[:,1])!=self.lado_inicial)[0])>0:
                ...
                ##otro carril
                print('Se cambio el carril, pero continuo')
                aseguir1=value[ind1]
                if len(value[np.argmax(abs(value[:,1]))])>0:
            ...
            if np.sign(value[np.argmax(value[:,2]),1])==self.lado_inicial:##self.lado_inicial correcto
                #self.lado_inicial=np.sign(value[np.argmax(value[:,2]),1])##izquierda -1 derecha 1#Actualizar
