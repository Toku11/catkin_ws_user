...
class image_converter:
  ...
  def callback(self,data):
    ...
    if (self.conta%2==0):
        ...
        tam_crop=12
        tam_crop2=0
        crop_img = frame[rows-tam_crop_g:rows-9, tam_crop2:cols-tam_crop2]
        ...
        cv2.line(sobelx,(-7,0),(13,85),(0,0,0),3)    
        #cv2.line(sobelx,(-7,0),(14,85),(255,255,255),2)    
        cv2.line(sobelx,(98,85),(121,0),(0,0,0),3)
    ...
    #***********************
        ...
        for idx in arreglo[1]:
            ...
            if init_flag==False:
                ...
                if new_p[1]==aux[1] and (new_p[0]<aux[0]+8 and new_p[0]>aux[0]-8):
                    valor.append(new_p[0])
                else: #new_p[1]>aux[1]:
                    if len(valor)>0:## promedio para un solo punto
                        for i in range(0,len(valor)):
        ...
        for val in ls:
            if len(ordp[0])==0:#lista nueva no inicializad
                ...
                flag=False
                for i in range(0,len(ordp)):
                    if np.abs(val[0]-ordp[i][len(ordp[i])-1][0])<8 and np.abs(val[1]-ordp[i][len(ordp[i])-1][1])<=(rws[0]-rws[4]):
                        ordp[i].append(val)
                        flag=True
                        break
                    elif i==len(ordp)-1 and flag==False:
        ...
        #################################################################3                    
        ######SE DESCARTAN LINEAS
        tam_carril=33##Es el tamaño de carril a detectar y a dibujar si o existe otro
        value=[]
        holg=8
        flg2=True
        n=0                  
