...
class image_converter:
  ...
  def callback(self,data):
    ...
    if (self.conta%4==0):
        ...
        #gray=cv2.warpPerspective(gray,M,(cols3+(correct*2)+50,rows3+elong+10))
        # tol=2
        # # Take each frame
        # ent=np.asarray([0,255])
        # out=np.asarray([0,255])
        # H,X1 = np.histogram( gray,255,[0,255])
        # F1 = np.cumsum(H)#*dx
        # total=gray.shape[0]*gray.shape[1]
        # low_bound=total*tol/100
        # upp_bound=total*(100-tol)/100
        
        # a=np.min(np.where(F1>=low_bound))
        # b=np.min(np.where(F1>=upp_bound))    
        # ent[0]=np.size(F1[0:a+1])
        # ent[1]=np.size(F1[0:b+1])
        # scale = np.float(out[1] - out[0]) / (np.float(ent[1] - ent[0])+sys.float_info.epsilon)
        ...
        # vs=(gray-ent[0])
        # vs[np.where(vs<0)]=0
        # vd=(vs*scale + 0.5)+out[0]
        # vd[np.where(vd>out[1])]=255
        # dst=vd.astype(np.uint8)
        #dst[np.where(dst>115)]=255
        #warped[np.where(np.logical_and(warped>95, warped<101))]=255
        #dst[np.where(dst<80)]=0
        #sobelx=cv2.Canny(dst,250,250,apertureSize = 3)
        gray=cv2.warpPerspective(gray,M,(cols3+(correct*2)+50,rows3+elong+10))
        gray = cv2.resize(gray, (200,105))
        theta=1.4#1.98
        maxIntensity=255.0
        phi=.6#.62
        ...
        ret, thresh = cv2.threshold(gray3,30,255,cv2.THRESH_BINARY_INV+cv2.THRESH_OTSU)
    ...
    #***********************
        ...
        #################################################################3                    
        ######SE DESCARTAN LINEAS
        tam_carril=67#Es el tamaño de carril a detectar y a dibujar si o existe otro
        value=[]
        holg=14
        n=0                  
        for x in range(0,len(ordp)):
      ...
      #########################################################################
        ...
        if len(value)>2:##tres carriles
            #if np.sign(value[np.argmax(value[:,2]),1])==lado:##lado correcto
            self.lado=np.sign(value[np.argmax(value[:,2]),1])##izquierda -1 derecha 1
            aseguir1=value[np.argmax(value[:,2])]
            sobrante=value[np.where(np.sign(value[:,1])!=self.lado)]##posible lineas a carril
            ...
            if len(extra)==0:##Extra fuera de rango 
                ...
                extra=extra[0]
            #elif np.sign(value[np.argmax(value[:,2]),1])!=lado
             #   sobrante=np.delete(value,ind1,0)
              #  if len(np.where((sobrante[:,1]<lado))[0])
               #         and np.sign(value[np.argmin(value[:,2]),1])==lado:
                
        elif len(value)==2:
            
            if np.sign(value[np.argmax(value[:,2]),1])==self.lado:##self.lado correcto
                self.lado=np.sign(value[np.argmax(value[:,2]),1])##izquierda -1 derecha 1#Actualizar
                aseguir1=value[np.argmax(value[:,2])]
                sobrante=value[np.where(np.sign(value[:,1])!=self.lado)]##posible lineas a carril
                sobrante[:,1]=sobrante[:,1]-value[np.argmax(value[:,2]),1]##tamaños de carril en [1]
                aseguir2=sobrante[np.where(np.all([abs(sobrante[:,1])>tam_carril-holg,abs(sobrante[:,1])<tam_carril+holg],0))]
            ...
            elif np.sign(value[np.argmax(value[:,2]),1])!=self.lado and np.sign(value[np.argmin(value[:,2]),1])==self.lado:
        ...
        if gen1==True:
            xx1=np.array([[aseguir2[3]+tam_carril*self.lado,ordp[indice][1][1]]])
        if gen2==True:
            xx=np.array([[aseguir1[3]+tam_carril*self.lado*-1,ordp[int(aseguir1[0])][1][1]]])
        if genextra==True:
            xx=np.array([[aseguir1[3]+tam_carril*self.lado*-2,ordp[int(aseguir1[0])][1][1]]])
        ...
        if rside==True:
            b_b=abs(xx1[0][0]-xx[0][0])/2*self.lado*-1
        else:
            b_b=abs(xx1[0][0]-xx[0][0])/2*self.lado
