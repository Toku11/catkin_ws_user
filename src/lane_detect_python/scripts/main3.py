...
class image_converter:
  ...
  def callback(self,data):
    ...
    if (self.conta%4==0):
        ...
        #ret, thresh = cv2.threshold(gray4,200,255,cv2.THRESH_BINARY+cv2.THRESH_OTSU)
        ret, thresh = cv2.threshold(gray4,210,255,cv2.THRESH_BINARY+cv2.THRESH_OTSU)
        ...
        sobelx = cv2.resize(sobelx, (200,105))

        cv2.line(sobelx,(0,14),(59,105),(0,0,0),6)    
        #cv2.line(sobelx,(0,77),(21,96),(255,255,255),2)    
        cv2.line(sobelx,(200,14),(141,105),(0,0,0),6)
    ...
    #***********************
        ...
        n=0                  
        for x in range(0,len(ordp)):
            if (ordp[x][0][1]>40) and (len(ordp[x])>7):##LIMITE Y y LIMITE DE TAMAÑO
                value.append([x,(ordp[x][1][0]-centro),len(ordp[x]),ordp[x][1][0]])
