...
class image_converter:
  ...
  def callback(self,data):
    ...
    if (self.conta%4==0):
        ...
        gray=cv2.warpPerspective(dst,M,(cols3+(correct*2)+50,rows3+elong+10))
        ...
        #sobelx = sobelx[40:rowss, 0:colss]
        ...
        for val in ls:
            if len(ordp[0])==0:#lista nueva no inicializad
                ...
                for i in range(0,len(ordp)):
                    if np.abs(val[0]-ordp[i][len(ordp[i])-1][0])<20 and np.abs(val[1]-ordp[i][len(ordp[i])-1][1])<=(rws[0]-rws[7]):##espacio entre lineas
                        ordp[i].append(val)
                        flag=True
                        break
        ...
        #################################################################3                    
        ######SE DESCARTAN LINEAS
        tam_carril=58##Es el tamaño de carril a detectar y a dibujar si o existe otro
        value=[]
        holg=17
        flg2=True
        n=0                  
        for x in range(0,len(ordp)):
            if (ordp[x][0][1]>10) and (len(ordp[x])>6):##LIMITE Y y LIMITE DE TAMAÑO
